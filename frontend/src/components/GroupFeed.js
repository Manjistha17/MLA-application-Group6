...
const icons = {
  ...
  AWARDED_BADGE: (
    <svg xmlns="http://www.w3.org/2000/svg" fill="#ff9800" viewBox="0 0 24 24">
      <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
    </svg>
  ),
  CLOCK: (
...
const GroupFeedDashboard = ({ currentUser }) => {
...
useEffect(() => {
  const fetchGroups = async () => {
    ...
    try {
      // Use currentUser from props instead of userId
      const res = await axios.get(`http://16.171.162.5:8005/users/${currentUser}/groups`);
      setGroups(res.data);
      if (res.data.length > 0) setSelectedGroup(res.data[0].groupId); // default first group
    } catch (err) {
  ...
  if (currentUser) fetchGroups(); // only fetch if currentUser exists
}, [currentUser]); // dependency updated

  // Fetch feed for the selected group
  useEffect(() => {
  ...
  return (
    <div className="group-feed-dashboard">
      <h2>User Groups</h2>
...
export default GroupFeedDashboard;