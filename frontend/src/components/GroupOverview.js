...
const GroupDashboard = ({ currentUser, onNav }) => {
  ...
  useEffect(() => {
    axios.get(`/users/${currentUser}/groups`)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const gRes = await axios.get(`/groups/${selectedGroup}`);
        const g = gRes.data;
        ...
        const [fRes, lRes, pRes] = await Promise.all([
          axios.get(`/groups/${selectedGroup}/feed`),
          axios.get(`/groups/${selectedGroup}/leaderboard`, { params: { top_n: 10, metric } }),
          axios.get(`/groups/${selectedGroup}/progress`),
        ]);
  ...
  const handleMembersToggle = async () => {
    ...
    try {
      const res = await axios.get(`/groups/${selectedGroup}/members`);
      setMembers(res.data || []);
    } catch { setMembers([]); }
  };
