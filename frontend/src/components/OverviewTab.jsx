...
// ── OverviewTab now accepts tipVersion from the parent ──
const OverviewTab = ({ currentUser, onNavigate, tipVersion }) => {
  const [stats, setStats] = useState({
    caloriesBurned: 0, activeMinutes: 0, streak: 0, activities: [],
  });
  const [nutrition, setNutrition] = useState({
    calories: 0, water: 0, protein: 0, carbs: 0, fat: 0,
  });
  ...
  useEffect(() => {
    ...
    axios.get(`https://d393qv373r18to.cloudfront.net/exercises/weekly/${currentUser}`)
      .then((res) => setWeeklyData(res.data))
      .catch(() => {});

  }, [currentUser, today, tipVersion]); // re-fetch overview stats when tip refreshes too
  ...
  const statCards = [
    ...
    null, // workouts card — rendered separately below
    ...
    <Box sx={{ width: "100%" }}>
                    ...
                    <Stack direction="row" flexWrap="wrap" gap={0.5} mt={1}>
                      {stats.activities.map((act, i) => (
                        <Box
                          key={i}
                          sx={{
                            px: 1, py: 0.4, fontSize: "0.72rem", borderRadius: "999px",
                            border: `1px solid ${c.color}`, color: c.color, bgcolor: c.bg, fontWeight: 500,
                          }}
                        >
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 2 }}>
        ...
        <Card
          ...
          sx={{ ...cardSx, cursor: "pointer", "&:hover": { boxShadow: "var(--shadow-md)", transform: "translateY(-2px)" } }}
        >
          <CardContent>
            ...
            <Stack spacing={3}>
              <Box>
                ...
                <Typography variant="h4" fontWeight={700} sx={{ color: "var(--color-text-primary)" }}>
                  {nutrition.calories}
                  <Typography component="span" variant="body1" sx={{ color: "var(--color-text-secondary)" }} ml={1}>
                    / {calorieGoal} kcal
                  </Typography>
                </Typography>
                <LinearProgress variant="determinate" value={caloriePercent} color={calorieColor} sx={{ height: 8, borderRadius: 5, mt: 1 }} />
                <Typography variant="caption" sx={{ color: "var(--color-text-secondary)" }}>
                  {Math.max(0, calorieGoal - nutrition.calories)} kcal remaining
                </Typography>
              ...
              <Box>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  ...
                  <Typography variant="body2" fontWeight={600} sx={{ color: "var(--color-text-secondary)" }}>
                ...
                <Typography variant="caption" sx={{ color: "var(--color-text-secondary)" }}>
                  {Math.max(0, waterGoal - nutrition.water)} ml remaining
                </Typography>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={weeklyData}>
                ...
                <Area
                  ...
                  fill="url(#blueGrad)" dot={{ r: 4, fill: "#3b82f6", strokeWidth: 0 }} activeDot={{ r: 7 }}
                />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
      ...
      {/* ── Row 3 — AI Coach ── */}
      <AICoach currentUser={currentUser} tipVersion={tipVersion} />
